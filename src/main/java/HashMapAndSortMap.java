...
// java离线扩容技术  jvm离线申请扩容  不占用在线内存
    ...
    //32位无符号整数的范围是0~4,294,967,295，现在有40亿个数
    //1g内存 找出所有未出现过的数
    //内存限制为 10MB，但是只用找到一个没出现过的数即可

    //0-2^32范围  40亿数  2^32/8字节约等于500M来表示所有的数 是否存在
    //0-2^32范围  分512份  每一份有一个范围  把一个数除以范围  在512份中的某一份中加一 总会有不满的  再从不满的继续分 直到找出那个数

//搜索百亿top  hash分文件 再使用大小堆


    //10g的数  需要你用5g内存进行排序  ：分范围 再使用大根堆
